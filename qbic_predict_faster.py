...
# Worker-process globals no longer needed — batched computation replaces per-variant parallelism
...
def load_model_id_map(map_path: str) -> Dict[str, int]:
    """Load model-to-ID mapping from a CSV file generated by generate_model_map.py."""
    import csv
    model_id_map = {}
    with open(map_path, 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            model_id_map[row['model_name']] = int(row['model_id'])
    return model_id_map

def round_sig(series: pd.Series, sig: int = 4) -> pd.Series:
    """Round a pandas Series to `sig` significant digits, preserving NaN and zero."""
    def _round(x):
        if pd.isna(x) or x == 0:
            return x
        return float(f'{x:.{sig-1}e}')
    return series.map(_round)

# Column name aliases: canonical lowercase → possible input names
_COLUMN_ALIASES = {
...
def apply_short_output(df: pd.DataFrame, model_id_map: Dict[str, int], compute_stats: bool) -> pd.DataFrame:
    """Transform a results DataFrame to concise short output format."""
    result = df.copy()

    # Rename variant columns to canonical lowercase names
    rename_map = {}
    ...
    # Replace model name with integer ID
    result['model'] = result['model'].map(model_id_map)

    # Round numeric columns
    result['predicted_effect'] = round_sig(result['predicted_effect'], 4)
    if compute_stats and 'z_score' in result.columns:
        result['z_score'] = round_sig(result['z_score'], 4)
        result['p_value'] = round_sig(result['p_value'], 4)

    # Select only desired columns
    cols = ['chrom', 'pos', 'ref', 'alt', 'model', 'predicted_effect']
    if compute_stats and 'z_score' in result.columns:
...
def count_sequences_with_n(variants_df: pd.DataFrame) -> int:
    ...
    for _, row in variants_df.iterrows():
        ...
        if ref_seq is None or alt_seq is None:
            na_count += 1
        elif 'N' in ref_seq or 'N' in alt_seq:
            na_count += 1
...
def _build_c_matrix(alt_list: List[List[str]], ref_list: List[List[str]],
                    partial_dict: Dict[str, float],
                    start: int, end: int) -> Tuple[np.ndarray, List[int]]:
    """Build the coefficient matrix C for a chunk of variants.

    C[i, j] = (count of partial_key[j] in alt k-mers) - (count in ref k-mers),
    matching the per-variant logic of the original convert_kmers_to_c_vectorized.

    Returns (C, bad_rows) where bad_rows are local indices that raised exceptions.
    """
    ...
    C = np.zeros((chunk_n, d), dtype=np.int8)
    bad_rows = []

    for local_i in range(chunk_n):
        global_i = start + local_i
        try:
            for kmer in alt_list[global_i]:
                idx = key_to_idx.get(kmer)
                if idx is None:
                    rc_kmer = utils.reverse_complement(kmer)
                    idx = key_to_idx.get(rc_kmer)
                if idx is not None:
                    C[local_i, idx] += 1

            for kmer in ref_list[global_i]:
                idx = key_to_idx.get(kmer)
                if idx is None:
                    rc_kmer = utils.reverse_complement(kmer)
                    idx = key_to_idx.get(rc_kmer)
            ...
            bad_rows.append(local_i)

    return C, bad_rows


def _apply_edge_cases(t_chunk: np.ndarray, p_chunk: np.ndarray,
                      estimates: np.ndarray, se_all: np.ndarray,
                      bad_rows: List[int]) -> None:
    """Apply edge-case masks in-place to match original per-variant behaviour.

    - estimate == 0  →  (t=0, p=1)
    - se invalid     →  (t=nan, p=nan)
    - bad_rows       →  (t=nan, p=nan)
    """
...
def compute_statistics_batched_cpu(alt_list: List[List[str]], ref_list: List[List[str]],
                                   partial_dict: Dict[str, float], cov_matrix: np.ndarray,
                                   model_name: str = None,
                                   chunk_size: int = 20_000) -> Tuple[List[float], List[float]]:
    """Batched CPU statistics — replaces per-variant loop with matrix operations.

    Mathematically identical to the original:
      estimate_i = c_i^T @ lm_values
      se_i       = sqrt(c_i^T @ cov @ c_i)
      t_i        = estimate_i / se_i
      p_i        = 2 * norm.sf(|t_i|)

    The key identity: row_sum((C @ cov) * C)[i] == c_i^T @ cov @ c_i
    """
    lm_values = np.array(list(partial_dict.values()), dtype=np.float32)
    cov = cov_matrix.astype(np.float32, copy=False)
    n_variants = len(alt_list)

    t_scores = np.full(n_variants, np.nan, dtype=np.float64)
    p_values = np.full(n_variants, np.nan, dtype=np.float64)
    error_count = 0

    n_chunks = (n_variants + chunk_size - 1) // chunk_size
    chunk_iter = range(0, n_variants, chunk_size)
    if TQDM_AVAILABLE:
        chunk_iter = tqdm(chunk_iter, total=n_chunks, desc="Computing statistics (batched CPU)", unit="chunk")
    ...
    if error_count > 0:
        print(f"Warning: {error_count}/{n_variants} variants failed during statistics computation")

    return t_scores.tolist(), p_values.tolist()


def compute_statistics_batched_gpu(alt_list: List[List[str]], ref_list: List[List[str]],
                                   partial_dict: Dict[str, float], cov_matrix: np.ndarray,
                                   model_name: str = None,
                                   chunk_size: int = 100_000) -> Tuple[List[float], List[float]]:
    """Batched GPU statistics — massive speedup over per-variant GPU loop.

    Same math as batched CPU, but C @ cov is done on GPU via cupy.
    C matrix is built on CPU (k-mer string ops aren't GPU-friendly).
    """
    if not GPU_AVAILABLE:
        raise RuntimeError("GPU not available for processing")

    lm_values_gpu = cp.array(list(partial_dict.values()), dtype=cp.float32)
    ...
    chunk_iter = range(0, n_variants, chunk_size)
    if TQDM_AVAILABLE:
        chunk_iter = tqdm(chunk_iter, total=n_chunks, desc="Computing statistics (batched GPU)", unit="chunk")

    for start in chunk_iter:
        end = min(start + chunk_size, n_variants)

        # Build C on CPU (string operations)
        C, bad_rows = _build_c_matrix(alt_list, ref_list, partial_dict, start, end)
        error_count += len(bad_rows)
        ...
        estimates_gpu = C_gpu @ lm_values_gpu                  # (chunk_n,)
        C_cov_gpu = C_gpu @ cov_gpu                            # (chunk_n, d)
        se_gpu = cp.sqrt(cp.sum(C_cov_gpu * C_gpu, axis=1))   # (chunk_n,)

        with np.errstate(divide='ignore', invalid='ignore'):
            t_gpu = estimates_gpu / se_gpu

        # Pull back to CPU for p-value (scipy.stats not on GPU)
        t_chunk = cp.asnumpy(t_gpu).astype(np.float64)
        estimates_np = cp.asnumpy(estimates_gpu)
        se_np = cp.asnumpy(se_gpu)
        p_chunk = 2.0 * stats.norm.sf(np.abs(t_chunk))

        _apply_edge_cases(t_chunk, p_chunk, estimates_np, se_np, bad_rows)

        t_scores[start:end] = t_chunk
        p_values[start:end] = p_chunk

        del C_gpu, C_cov_gpu, estimates_gpu, se_gpu, t_gpu
...
def compute_statistics_parallel(alt_list: List[List[str]], ref_list: List[List[str]],
    ...
    """Compute statistics using batched matrix operations (GPU or CPU).

    Drop-in replacement for the original per-variant loop. The function
    signature is unchanged so all callers work without modification.
    n_jobs is accepted for interface compatibility but ignored — batched
    BLAS already uses all cores via numpy/MKL threading.
    """
    if use_gpu and GPU_AVAILABLE:
        try:
            return compute_statistics_batched_gpu(alt_list, ref_list, partial_dict, cov_matrix, model_name)
        except Exception as e:
    ...
    print(f"Using batched CPU computation (BLAS-accelerated)")
    return compute_statistics_batched_cpu(alt_list, ref_list, partial_dict, cov_matrix, model_name)
...
def predict_batch_models(models_file: str, variants_df: pd.DataFrame,
                        compute_stats: bool = False, cov_file: Optional[str] = None,
                        use_gpu: bool = True, genome: str = "hg38",
                        wildcard: Optional[str] = None, n_jobs: int = -1,
                        pval_threshold: Optional[float] = None,
                        output_path: Optional[str] = None,
                        output_dir_mode: bool = False,
                        short_output: bool = False,
                        model_id_map: Optional[Dict[str, int]] = None) -> Dict:
    """Predict variant effects using models specified in a text file.

    Processes models sequentially with memory-efficient incremental output:
    - Filters by p-value per model (if pval_threshold is set) before accumulating
    - Writes results to disk incrementally instead of holding all in memory
    - Clears model/covariance caches after each model to free memory

    Returns a summary dict with keys: total_predictions, total_before_filter,
    failed_models, has_stats, models_processed.
    """
    print("\nVALIDATION")
    print("-" * 50)
    ...
    if output_dir_mode and output_path:
    ...
    for model_path in model_paths:
        ...
        try:
            ...
            if result is not None:
                ...
                # Check if this model produced statistics
                if 'z_score' in result.columns:
                    has_stats = True

                # Apply p-value filter per model to reduce memory
                if pval_threshold is not None and 'p_value' in result.columns:
                    result = result[result['p_value'].notna() & (result['p_value'] <= pval_threshold)]
                    print(f"  P-value filter: kept {len(result)} / {before_count} predictions")

                # Apply short output transformation
                if short_output and model_id_map is not None:
                    result = apply_short_output(result, model_id_map, compute_stats)
                ...
                if output_path and len(result) > 0:
                    if output_dir_mode:
                        # One CSV per model
                        model_output = Path(output_path) / f"{model_name}.csv"
                        result.to_csv(model_output, index=False)
                        print(f"  Saved to: {model_output.name}")
                    else:
                        ...
                        if not header_written:
                            ...
                            result.to_csv(output_path, index=False, mode='a', header=False)

                # Free this model's results from memory
                del result
            else:
        ...
        # Clear caches after each model - models are processed sequentially
        # so cached data from previous models is never reused
        _model_cache.clear()
        _cov_cache.clear()
    ...
    print(f"Batch processing complete! Total predictions: {total_predictions}")
    if pval_threshold is not None:
        print(f"P-value filter (p <= {pval_threshold}): kept {total_predictions} / {total_before_filter}")
...
Input Formats:
  ...
  Pre-extracted sequences: ref_sequence,alt_sequence
    ...
    parser.add_argument("--short-output", action="store_true",
                       help="Concise output: minimal columns, reduced precision, integer model IDs")
    parser.add_argument("--model-map", type=str, default=None,
                       help="Path to model-to-ID mapping CSV (generated by generate_model_map.py, required with --short-output)")
    ...
    if args.short_output and args.model_map is None:
        parser.error("--short-output requires --model-map")
    if args.model_map is not None and not args.short_output:
        parser.error("--model-map requires --short-output")
    ...
    try:
        ...
        if args.short_output:
            print(f"   Short output: Enabled (model map: {args.model_map})")

        # Load model ID map if short output is enabled
        model_id_map = load_model_id_map(args.model_map) if args.short_output else None
        ...
        if not has_sequences:
            ...
            # Filter out variants where sequence extraction failed (e.g., ref allele mismatch)
            null_mask = variants_df['ref_sequence'].isnull() | variants_df['alt_sequence'].isnull()
            n_failed = null_mask.sum()
            if n_failed > 0:
                print(f"Dropped {n_failed} variants due to failed sequence extraction (ref allele mismatch)")
                variants_df = variants_df[~null_mask].reset_index(drop=True)
        else:
        ...
        if is_single_model:
            ...
            # P-value filtering (single model only - batch handles it internally)
            ...
            # Apply short output transformation
            if args.short_output:
                results = apply_short_output(results, model_id_map, args.compute_stats)
            ...
            # Batch mode: processes models incrementally with per-model p-value
            # filtering and disk writing to avoid accumulating all results in memory
            batch_summary = predict_batch_models(
                model_path, variants_df, args.compute_stats,
                args.cov_file, args.use_gpu, args.genome, args.wildcard, args.n_jobs,
                pval_threshold=args.pval,
                output_path=args.output,
                output_dir_mode=args.output_dir,
                short_output=args.short_output,
                model_id_map=model_id_map
            )

            # Output already written incrementally by predict_batch_models
            print("\nOUTPUT PROCESSING")
            print("-" * 50)
            if args.output_dir:
            ...
            # Summary
            elapsed_time = time.time() - start_time
            print(f"Successfully processed {batch_summary['total_predictions']} predictions in {elapsed_time:.2f} seconds")
            print(f"Models processed: {batch_summary['models_processed']}")

            if batch_summary['has_stats']:
                print(f"Statistics were computed for predictions")
